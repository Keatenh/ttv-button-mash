...
async function main() {
    ...
    const authProvider = new RefreshingAuthProvider(
        {
            ...
            onRefresh: async (newTokenData) => {
                await fs.writeFile(config.tokenFile, JSON.stringify(newTokenData, null, 4), 'utf8', _err => {});
    ...
    chatClient.onRegister(() => { 
        chatClient.say(channelName, "!sh3 - Chat Assist Enabled MrDestructoid - Redeem 'Mash Button' to help save the run!");
    });
    ...
    await listener.subscribeToChannelRedemptionAddEventsForReward(channelId, rewardId, async (event) => {
	    ...
	    let presses = 0;
        for (let i=0; i < config.requestedPresses; i++) {
            // Checking Window
            const windowRef = await getActiveWindow();
            const title = await Promise.resolve(windowRef.title);
            if (title === `${process.env.TPBM_WINDOW_TITLE}` || process.env.TESTING_TTVBM === "true") {
